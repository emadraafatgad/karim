...
class AccountInvoice(models.Model):
    ...
    discount_type = fields.Selection([('line', 'Order Line'), ('global', 'Global')], 'Discount Applies to',default='line')
    ...
    amount_due_string = fields.Char()
    amount_due_value = fields.Float()
        ...
        if self._context.get('default_type') == 'out_invoice' :
            ...
            for line in self.invoice_line_ids:
                ...
                if line.discount_method == 'per':
                    ...
                    total += (line.discount_amt * line.quantity)
        ...
        for line in self.invoice_line_ids:
            if self.discount_type == 'line':
                if line.discount_method == 'fix':
                    ...
                    line.discount_subtotal = discount_price * line.quantity
                elif line.discount_method == 'per':
                    ...
                    line.discount_subtotal = price_x
...
class account_move_line(models.Model):
    ...
    #     for line in self:
    #         if line.move_id.discount_type == 'line':
    #             if line.discount_method == 'fix':
    #                 if line.price_unit < line.discount_amount:
    #                     raise ValidationError(_("Unit Price should be greater than Discount Amount !"))
    ...
    #             if line.discount_method == 'fix':
    #                 if line.price_unit < line.discount_amount:
    #                     raise ValidationError(_("Discount Amount should be less than Unit Price !"))

    @api.onchange('discount_method', 'discount_type', 'discount_amount', 'price_unit', 'quantity')
    def calculate_discount_unit_price(self):
        for line in self:
            if line.move_id.discount_type == 'line':
                if line.discount_method == 'fix':
                    if line.price_unit < line.discount_amount:
                        raise ValidationError(_("Unit Price should be greater than Discount Amount !"))
                    discount_price = line.price_unit - line.discount_amount
                    line.update({'discount_unit_price':discount_price,
                                'discount_amt': line.discount_amount,
                                'discount_subtotal': discount_price * line.quantity})
                elif line.discount_method == 'per':
                    discount_price = (line.discount_amount/100) * line.price_unit
                    disc_price = line.price_unit - discount_price
                    price_x = 0
                    price_x = ((line.price_unit * line.quantity) - (line.price_unit * line.quantity) * (1 - (line.discount_amount or 0.0) / 100.0))
                    line.update({'discount_unit_price':disc_price,
                                'discount_amt': price_x,
                                'discount_subtotal': disc_price * line.quantity})


    @api.onchange('discount_unit_price')
    def calculate_discount_amt(self):
        for rec in self:
            if rec.discount_method:
                if rec.discount_unit_price > rec.price_unit:
                    rec.discount_unit_price = rec.discount_subtotal/rec.quantity if rec.quantity else 0
                    return {'warning' : {
                            'title': _("Warning"),
