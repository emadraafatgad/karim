...
class InventoryBreakdown(models.Model):
    ...
    generate_type = fields.Selection([('warehouse','Warehouse'),('location','Location')],'Report Based On',default='warehouse')
    ...
    def get_product_stock(self):
        ...
        if self.breakdown_days_ids:
            for days in self.breakdown_days_ids:
                ...
                product_obj = self.env['product.product'].search([('id','in',ware_loc_list),('write_date','>=',str(en)),('write_date','<=',str(st))])
                if product_obj:
    ...
    def print_exl_report(self):
            ...
            for quants in get_product_stock:
                ...
                if rec_c == 1:
                    ...
                    for pro_id in quants:
                        ...
                        if pro_id.stock_value:
                            total_value += pro_id.stock_value
                    for quant in quants:
                        ...
                        if quant.qty_available:
                            ...
                            worksheet.write(row, col+2, qty_per_inv, style)
                        
                        worksheet.write(row, col+3, quant.stock_value, style)
                        if quant.stock_value:
                            #total_value += quant.stock_value
                            value_per_inv = ((quant.stock_value * 100)/total_value)
                ...
                if rec_c == 2:
                    ...
                    for quant in quants:
                        ...
                        if quant.qty_available:
                            total_qty += quant.qty_available
                            qty_per_inv = ((quant.qty_available * 100)/total_qty)
                            worksheet.write(row, 7, qty_per_inv, style)
                        worksheet.write(row, 8, quant.stock_value, style)
                        if quant.stock_value:
                            total_value += quant.stock_value
                            value_per_inv = ((quant.stock_value * 100)/total_value)
                ...
                if rec_c == 3:
                    ...
                    for quant in quants:
                        ...
                        if quant.qty_available:
                            ...
                            qty_per_inv = ((quant.qty_available * 100)/total_qty)
                            worksheet.write(row, 7, qty_per_inv, style)
                        worksheet.write(row, 13, quant.stock_value, style)
                        if quant.stock_value:
                            total_value += quant.stock_value
                            value_per_inv = ((quant.stock_value * 100)/total_value)
                            worksheet.write(row, 9, value_per_inv, style)
                ...
                if rec_c == 4:
                    ...
                    for quant in quants:
                        ...
                        if quant.qty_available:
                            ...
                            qty_per_inv = ((quant.qty_available * 100)/total_qty)
                            worksheet.write(row, 7, qty_per_inv, style)
                        worksheet.write(row, 18, quant.stock_value, style)
                        if quant.stock_value:
                            total_value += quant.stock_value
                            value_per_inv = ((quant.stock_value * 100)/total_value)
                            worksheet.write(row, 9, value_per_inv, style)
        ...
        fp = io.BytesIO()
        workbook.save(fp)
        export_id = self.env['inventory.age.report.excel'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
        res = {
                        'view_mode': 'form',
                        'res_id': export_id.id,
