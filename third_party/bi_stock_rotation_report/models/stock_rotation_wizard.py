	...
	def generate_xls_report(self):
		...
		if self.warehouse_ids:
			for ware in self.warehouse_ids:
				ware_list.append(ware.lot_stock_id.id)
				move_obj = self.env['stock.move'].search([('location_id','in',ware_list)])
			for move in move_obj:
				if move.picking_type_id.code == 'outgoing':
					sale.append(move.id)
					move_line.update({'outgoing':sale})
				...
				if move.picking_type_id.code == 'incoming':
					...
					move_line.update({'incoming':purchase})
					for move_l in move_line['incoming']:
						if self.env['stock.move'].browse(move_l).product_id in products_pur:
							temp = 	products_pur[self.env['stock.move'].browse(move_l).product_id]
							temp.append(move_l)
							products_pur[self.env['stock.move'].browse(move_l).product_id] = temp
						else:
							products_pur.setdefault(self.env['stock.move'].browse(move_l).product_id, []).append(move_l)
					for k,v in products_pur.items():
						products_pur.update({k:list(set(v))})
		...
		if products:
			...
			for k in products:
				...
				worksheet.write(prod_row, prod_col+8,str(last_sale), style)
		...
		if products_pur:
			...
			for k in products_pur:
				...
				worksheet.write(prod_row, prod_col+11, str(last_sale), style)
